 ...
 * 
 * - RESULT SET 은 0 개 이상의 행을 포함할 수 있다.
...
-- EMAIL 에서 _ 앞의 글자가 세글자인 사원 이름, 이메일 조회
SELECT EMP_NAME, EMAIL
FROM EMPLOYEE
WHERE EMAIL LIKE '____%';
-- 문제점 : _를 기준점으로 삼았으나, 패턴 중 '_'와 동일한 표기법으로 작성되어 구분되지 않음
-- 해결방법 : LIKE 의 ESCAPE OPTION을 이용하여 _를 구분한다.
-- LIKE 의 ESCAPE OPTION : 일반 문자로 처리할 '_' / '%' 앞에 아무 특수기호나 첨부해서 구분하게 함.
